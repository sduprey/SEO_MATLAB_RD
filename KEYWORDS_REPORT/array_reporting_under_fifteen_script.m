...
coverage_rate_local_per_entity_per_magasin=coverage_rate_per_entity_per_magasin./percentage_matrix*100;


%% Reporting keywords per magasin
header = {'Percentage of keywords'};
row_header=my_magasin;
whole_magasin_table = mat2rpttbl( percentage_keywords_per_magasin, header, row_header', []);
whole_magasin_table{1}='Magasin';
disp(whole_magasin_table);
...
for mag=1:length(my_magasin)
    ...
    if (strcmp('maison',loc_magasin))
        local_local_cov_rat=local_local_cov_rat-50;
    end
